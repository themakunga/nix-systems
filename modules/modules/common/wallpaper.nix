...
#              • wallpaper-apply — oneshot que llama a `swww img` una vez el daemon listo
{
  ...
  in {
    ...
    config = mkIf cfg.enable {
      ...
      environment.systemPackages = lib.mkIf (!isDarwin) [swwwPkg];

      # Daemon swww: arranca junto con la sesión gráfica y se mantiene vivo.
      systemd.user.services.swww-daemon = lib.mkIf (!isDarwin) {
      ...
      systemd.user.services.wallpaper-apply = lib.mkIf (!isDarwin) {
