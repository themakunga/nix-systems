...
{lib, ...}: {
  flake.nixosModules.hyprland-desktop = {
    ...
    #
    # Aquamarine 0.11 uses autodetection; WLR_* device overrides do not select its GPU.
  ...
  in {
    options.my.hyprland-desktop = {
      ...
      manageConfig = mkOption {
        ...
        default = true;
        description = ''
          Si true (por defecto), el módulo gestiona Hyprland y Waybar desde el Nix store.
          Establecer en false cuando los dotfiles (stow) gestionan ambos paquetes.
        '';
      };
    ...
    config = mkIf cfg.enable (lib.mkMerge [
        ...
        system.activationScripts = {
          "hyprland-config-${cfg.user}" = {
            ...
            text = ''
              ...
              # Hyprland 0.55.4 prefers hyprland.lua when choosing the default config.
          ...
          # Retirar solo enlaces generados por este módulo antes de entregar Waybar a Stow.
          waybar-stow-migration = lib.mkIf (!cfg.manageConfig) {
            deps = ["users"];
            text = ''
              for file in config style.css; do
                target="${config.users.users.${cfg.user}.home}/.config/waybar/$file"
                case "$(readlink "$target")" in
                  /nix/store/*-waybar-config.json|/nix/store/*-waybar-style.css)
                    unlink "$target"
                    ;;
                esac
              done
          ...
          stowDotfiles = lib.mkIf (!cfg.manageConfig) {
            deps = ["waybar-stow-migration"];
          };
        };
      }
      ...
      (lib.mkIf cfg.vnc.enable {
        ...
        systemd.user.services.wayvnc = {
          description = "WayVNC server — mirror de HDMI-A-1";
          ...
          serviceConfig = {
            ...
            ExecStartPre = pkgs.writeShellScript "wayvnc-pre" ''
              ...
              # Esperar a que Hyprland complete la inicialización
              sleep 2
            '';
            # Captura HDMI-A-1 directamente — sin output headless.
            # Un solo escritorio visible: VNC es un mirror del monitor físico.
            ExecStart = "${pkgs.wayvnc}/bin/wayvnc --output HDMI-A-1 ${cfg.vnc.address} ${toString cfg.vnc.port}";
          };
          environment = {
